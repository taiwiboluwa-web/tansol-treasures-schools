  ...
  return <main>
    <section className="page-shell grid min-h-[72vh] items-center gap-12 py-16 lg:grid-cols-[1.1fr_.9fr]">
      <div>
        ...
        <div className="mt-9 flex flex-wrap gap-3"><Link href="/admission" className="rounded-full bg-[var(--brand)] px-6 py-3 font-bold text-white transition hover:bg-[var(--brand-dark)]">Explore admission</Link><Link href="/portal" className="rounded-full border border-[var(--line)] bg-white px-6 py-3 font-bold transition hover:border-[var(--brand)] hover:text-[var(--brand)]">Enter school portal</Link></div>
      </div>
      <div className="relative min-h-[420px] overflow-hidden rounded-[2rem] border border-[var(--line)] bg-[var(--brand-soft)] p-8 text-[var(--ink)]">
        <div className="absolute -right-20 -top-20 size-64 rounded-full border border-[var(--brand)]/15" />
        <div className="absolute bottom-8 left-8 right-8 grid grid-cols-[1fr_auto] items-end gap-6 border-t border-[var(--brand)]/15 pt-6"><div><p className="eyebrow text-[var(--brand)]/70">Our promise</p><p className="display mt-2 text-3xl">Strong roots. Wider horizons.</p></div><span className="text-5xl font-light text-[var(--brand)]">01</span></div>
      </div>
    </section>
